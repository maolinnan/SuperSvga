...
class SVGAParser(private val context: Context) {
    ...
    fun parse(assetsName: String, callback: ParseCompletion,customLoaderFlow:Boolean) {
        this.isCustomLoaderFlow = customLoaderFlow
        parse(assetsName,callback)
    }
        ...
        if (bytes.size > 4 && bytes[0].toInt() == 80 && bytes[1].toInt() == 75 && bytes[2].toInt() == 3 && bytes[3].toInt() == 4) {
            ...
            try {
                if (isCustomLoaderFlow) {
                    inflate(bytes)?.let {
                        return SVGAVideoEntity(MovieEntity.ADAPTER.decode(it), File(cacheKey),true)
                    }
                }else{
                    inflate(bytes)?.let {
